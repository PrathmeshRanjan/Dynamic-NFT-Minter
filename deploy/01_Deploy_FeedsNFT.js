...
module.exports = async ({ getNamedAccounts, deployments, getChainId }) => {
  ...
  let highValue = "500000"; // $0.5

  log("----------------------------------------------------");
  let lowSVG = await fs.readFileSync("./img/svg/thumbsdown.svg", {
    encoding: "utf8",
  });
  let highSVG = await fs.readFileSync("./img/svg/thumbsup.svg", {
    encoding: "utf8",
  });
  let _args = [ethUsdPriceFeed];
  ...
  const feedsNFTContract = new ethers.Contract(
    feedsNFT.address,
    FeedsNFT.interface,
    signer
  );
  ...
  log(
    `Verify with npx hardhat verify --network ${networkName} ${feedsNFT.address} "${ethUsdPriceFeed}"`
  );
  log("Adding low URI...");
  ...
  log(`Here is the NFT tokenURI: ${await feedsNFTContract.tokenURI(4)}`);
};
